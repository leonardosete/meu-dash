...
def test_process_upload_and_generate_reports(mock_get_date, mock_context_builder, mock_gerador_paginas, mock_analisar_csv, tmp_path, mock_dependencies):
    """
    ...
    mock_Report_from_fixture.query.order_by.return_value.first.return_value = mock_last_report
    ...
    # Act
    result = services.process_upload_and_generate_reports(**mock_dependencies, upload_folder=str(upload_folder), reports_folder=str(reports_folder), base_dir=str(tmp_path))

    # Assert
    assert mock_analisar_csv.call_count >= 1, "A função de análise de CSV deveria ter sido chamada pelo menos uma vez."
    assert mock_context_builder.build_dashboard_context.called, "O construtor de contexto deveria ter sido chamado."
    assert mock_gerador_paginas.gerar_resumo_executivo.called, "A geração do resumo executivo deveria ter sido chamada."
    assert mock_dependencies["db"].session.add.called, "Um novo relatório deveria ter sido adicionado à sessão do DB."
    assert mock_dependencies["db"].session.commit.called, "A sessão do DB deveria ter sido 'commitada'."
